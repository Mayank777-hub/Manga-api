const render = require("../models/Decoratives");
...
const getalltest = async function (req, res) {
    try {
        let query = {};
        const allmangadata = await render.find(query).select('-_id');
        if (Object.keys(req.query).length === 0) {
            return res.status(200).json({ source: "All", allmangadata });
        }
        ...
        const mapping = {
            ...
            ItemType: "ItemType",
            Color:"Color",
            Images: "Images",// alternate for Type
            Country: "Manufacturer.Country",//if user need sub types(genres) of a genre 
            FrameMaterial: "FrameMaterial", // if user just need type of a particular anime like sojo only
            Description: "Type.Aboutmanga",
            Animeurl: "Animeurl",
            Material:"Material"
        };
         ...
         let filterdata = allmangadata.map(doc => {
            const docObj = doc.toObject ? doc.toObject() : doc;
            const { Reviewers_detail, ...leftover } = docObj;
            
            return {
                "Theme": leftover.Theme || "",
                "Title": leftover.Title || "",
                "Anime": leftover.Anime || "",
                "DecorType": leftover.DecorType || "",
                "Manufacturer.Name": leftover.Manufacturer?.Name || "",
                "Handmade": leftover.Handmade ? "handmade" : "",
                "ItemType": leftover.ItemType || "",
                "Color": Array.isArray(leftover.Color) ? leftover.Color.join(" ") : (leftover.Color || ""),
                "Images": Array.isArray(leftover.Images) ? leftover.Images.join(" ") : "",
                "Manufacturer.Country": leftover.Manufacturer?.Country || "",
                "FrameMaterial": leftover.FrameMaterial || "",
                "Animeurl": leftover.Animeurl || "",
                "Material": leftover.Material || "",
                 ...leftover,
            };
        });
        ...
        if (req.query.minPrice) {
            ...
            filterdata = filterdata.filter(manga =>
                manga.Originalvalue >= minPrice
            );
        }
      ...
      if (req.query.maxPrice) {
            ...
            filterdata = filterdata.filter(manga => manga.Originalvalue <= maxPrice);
        }

         const excludeParams = ['page', 'limit', 'minPrice', 'maxPrice'];
        const hasFuzzyKey = Object.keys(req.query).some(key => mapping[key] && !excludeParams.includes(key));
        if (!hasFuzzyKey) {
            return res.status(200).json({ source: "PriceOnly", allmangadata: filterdata });
        }
        // for ( let Keys in req.query){
        //     if (mapping[Keys]) {
        //         query[mapping[Keys]] = { $regex: req.query[Keys], $options: "i" };
        //     }
        // }
        const searchresult = [];
        //.select("Type.Title Type.Author Type.Genre Type.Publishinghouses Type.Characters.Name");
        for (const key in req.query) {
            if (!mapping[key] || excludeParams.includes(key)) continue;
            ...
            const results = new Fuse(filterdata, Options);
            const Find = results.search(req.query[key]);
            searchresult.push(...Find.map(r => r.item))
        }
        //let searchcollection = "";//if(req.query.Title){
        //    searchcollection += req.query.Title + " "
        //}
        //if(req.query.Author){//    searchcollection += req.query.Author + " "//}
        //if(req.query.Publishinghouses){//    searchcollection += req.query.Publishinghouses + " "//}//if(req.query.Characters){ //    searchcollection += req.query.Characters + " "//}//searchcollection = searchcollection.trim();//const Find = results.search(req.query.Title); //  it can only be used for only for title for entire use we used large collection in one string 
        //const uniqueresult = [...new Set(searchresult)];   // here what we do is remove duplicate which store in search result like if user search using title then we get naruto if user search using name of anime then also naruto so we remove duplicate here.


        const uniqueresult = Array.from(
    new Map(
        searchresult.map(item => [
            item._id?.toString() || JSON.stringify(item), 
            item
        ])
    ).values()
);

        const cleanedResults =uniqueresult // uniqueresult.map(doc => {
        //    const cleanDoc = { ...doc };
        //    delete cleanDoc["Theme"];
        //    delete cleanDoc["Title"];
        //    delete cleanDoc["Anime"];
        //    delete cleanDoc["DecorType"];   // what we do here actually we created keywords for fuse searching like theme,Title but it is not part o it so we not to remove it from here so that it not go to mdb back
        //    delete cleanDoc["Manufacturer.Name"];
        //    delete cleanDoc["Handmade"];
        //    delete cleanDoc["ItemType"];
        //    delete cleanDoc["Color"];
        //    delete cleanDoc["Images"];
        //    delete cleanDoc["Manufacturer.Country"];
        //    delete cleanDoc["FrameMaterial"];
        //    delete cleanDoc["Animeurl"];
        //    delete cleanDoc["Material"];
        //    return cleanDoc;
      //  });
        ...
        const result = cleanedResults.slice(start,stop);

        const pagination = {
          currentPage: page,
          totalPages: Math.ceil(total / limit),
          totalItems: total,
          hasNextPage: stop < total,
          hasPrevPage: start > 0,
};
        res.status(200).json({ source: "fuzzy+Filter",pagination, allmangadata: result });
    }
